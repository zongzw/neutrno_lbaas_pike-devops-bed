...
class CreateListener(neutronV20.CreateCommand):
    ...
    def add_known_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help=_('Client authentication mode for the listener.'))
        parser.add_argument(
            '--client-ca-tls-container-ref',
            dest='client_ca_tls_container_ref',
            help=_('Client CA TLS container reference'
                   ' to retrieve TLS information.'))
        parser.add_argument(
            '--client-crl-container-ref',
            dest='client_crl_container_ref',
            help=_('Client CRL container reference'
                   ' to retrieve TLS information.'))
        parser.add_argument(
            '--tls-protocols',
            dest='tls_protocols',
            help=_('TLS protocols'))
        parser.add_argument(
            '--cipher-suites',
            dest='cipher_suites',
            help=_('Cipher suites'))
        parser.add_argument(
            '--sni-container-refs',
        ...
        parser.add_argument(
            '--http2',
            dest='http2',
            help=_('To enable/disable http2'))
        parser.add_argument(
            '--loadbalancer',
        ...
        parser.add_argument(
            '--protocol',
            required=True,
            choices=['TCP', 'UDP', 'FTP', 'HTTP', 'HTTPS', 'TERMINATED_HTTPS'],
            type=utils.convert_to_uppercase,
...
class UpdateListener(neutronV20.UpdateCommand):
    ...
    def args2body(self, parsed_args):
        body = {}
        neutronV20.update_dict(parsed_args, body,
                               ['admin_state_up', 'transparent', 'http2'])
        _parse_common_args(body, parsed_args, self.get_client())
        return {self.resource: body}
