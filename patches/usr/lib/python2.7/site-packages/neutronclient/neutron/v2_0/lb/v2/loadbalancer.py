...
def _add_common_args(parser):
    ...
    parser.add_argument(
        '--flavor',
        #dest='flavor',
        help=_('flavor of the load balancer.'))
    parser.add_argument(
        '--name', metavar='NAME',
    ...
    parser.add_argument(
        '--bandwidth',
        dest='bandwidth',
        help=_('Bandwidth for the loadbalancer.'))


def _parse_common_args(body, parsed_args):
    neutronV20.update_dict(parsed_args, body,
                           ['name', 'description', 'bandwidth', 'flavor'])
...
class ListLoadBalancer(neutronV20.ListCommand):
    ...
    list_columns = ['id', 'name', 'bandwidth', 'vip_address', 'flavor',
...
class CreateLoadBalancer(neutronV20.CreateCommand):
    ...
    def args2body(self, parsed_args):
        ...
        if 'flavor' not in body:
            if parsed_args.flavor:
                body['flavor'] = parsed_args.flavor

        if 'bandwidth' not in body.keys():
            body['bandwidth'] = 0
        neutronV20.update_dict(parsed_args, body,
                               ['provider', 'bandwidth', 'vip_address', 'tenant_id', 'flavor'])
